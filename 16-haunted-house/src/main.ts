...
const gui = new GUI()
const debugObject = {
  doorLightFlicker: true,
  doorLightIntensity: 3,
}
...
folderFog.add(fog, 'near').min(0).max(15).step(0.001)
folderFog.add(fog, 'far').min(0).max(15).step(0.001)
...
folderMoonLight.add(moonLight.position, 'x').min(-5).max(5).step(0.001).name('position.x')
folderMoonLight.add(moonLight.position, 'y').min(-5).max(5).step(0.001).name('position.y')
folderMoonLight.add(moonLight.position, 'z').min(-5).max(5).step(0.001).name('position.z')
scene.add(moonLight, moonLight.target)

const moonLightHelper = new THREE.DirectionalLightHelper(moonLight)
moonLightHelper.visible = false
folderMoonLight.add(moonLightHelper, 'visible').name('moonLightHelper')
...
folderDoorLight.add(debugObject, 'doorLightIntensity').min(0).max(3).step(0.001)
folderDoorLight.add(debugObject, 'doorLightFlicker')
house.add(doorLight)
...
// avoid camera going too close
controls.minDistance = 5
// avoid camera going too far
...
const tick = () => {
  ...
  // Door light flicker
  if (debugObject.doorLightFlicker) {
    doorLight.intensity = Math.abs(Math.sin(elapsedTime * 5) * Math.sin(elapsedTime * 3.5) * Math.sin(elapsedTime * 2.5))
  } else {
    doorLight.intensity = debugObject.doorLightIntensity
  }
